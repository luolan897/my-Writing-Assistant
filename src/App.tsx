...
function App() {
  const { docs, currentDocId, messages, aiSettings, knowledge, externalKnowledge, addDoc, updateDoc, renameDoc, deleteDoc, setCurrentDoc, addMessage, updateMessage, clearMessages, removeMessagesFrom, updateAISettings, appendToKnowledge, setExternalKnowledge, clearExternalKnowledge } = useStore()
  ...
  // 修改并重发逻辑
  const handleEditAndSend = async (index: number) => {
    if (loading) return
    updateMessage(index, editInput)
    const history = [...messages.slice(0, index), { ...messages[index], content: editInput }]
  ...
  const handleLoadExternal = () => {
    ...
    inp.onchange=async(e)=>{const f=(e.target as any).files[0]; if(f) { const data=JSON.parse(await f.text()); setExternalKnowledge(Array.isArray(data)?data:[]) }};
    inp.click();
  }
  ...
  return (
    <div className="app">
        ...
        <ul className="doc-list">
          {docs.map((doc) => (
            <li key={doc.id} className={doc.id === currentDocId ? 'active' : ''} onClick={() => setCurrentDoc(doc.id)}>
              {editingTitle === doc.id ? (
                <input autoFocus defaultValue={doc.title} onBlur={(e) => { renameDoc(doc.id, e.target.value); setEditingTitle(null) }} />
              ) : (
                <><span onDoubleClick={() => setEditingTitle(doc.id)}>{doc.title}</span><button onClick={(e) => {e.stopPropagation(); if(confirm('删除?')) deleteDoc(doc.id)}}>×</button></>
              )}
            </li>
          ))}
        </ul>
      ...
      <main className="main">
        {currentDoc ? (
          <><div className="editor-panel"><Editor content={currentDoc.content} onChange={(val) => updateDoc(currentDoc.id, val)} /></div>
            <div className="chat-panel">
                  ...
                  <div key={i} className={`message ${msg.role}`}>
                    {editingMsgIndex === i ? (
                      <div className="edit-area">
                        ...
                        <button onClick={() => handleEditAndSend(i)}>保存并重发</button>
                        <button onClick={() => setEditingMsgIndex(null)}>取消</button>
                      </div>
                    ) : (
                      <div className="message-content">
                        {msg.content}
                        {msg.role === 'user' && <button className="edit-btn-inline" onClick={()=>{setEditingMsgIndex(i); setEditInput(msg.content)}}>✏️</button>}
                      </div>
                    )}
                    {msg.role === 'assistant' && (
                      <div className="message-actions">
                        <button onClick={() => updateDoc(currentDoc.id, currentDoc.content + '<p>' + msg.content + '</p>')}>📝 插入</button>
                        <button onClick={() => handleRegenerate(i)}>🔄 重新生成</button>
                        <button onClick={() => setSaveDropdown(saveDropdown === `${i}` ? null : `${i}`)}>💾 存</button>
              ...
              {matchedKnowledge.length > 0 && <div className="matched-hint" style={{fontSize:'10px', color:'#999', padding:'5px'}}>📎 参考: {matchedKnowledge.length} 条设定</div>}
        ...
        ) : <div className="empty-state"><h2>✨ 写作助手</h2><button onClick={() => {const t=prompt('标题:'); if(t) addDoc(t)}}>开始创作</button></div>}
      </main>
        ...
        <div className="modal-overlay" onClick={() => setShowSettings(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="settings-section">
                <p>存储: {storageUsage} | 外部: {externalKnowledge.length}条</p>
                <button onClick={handleLoadExternal}>加载知识库</button>
